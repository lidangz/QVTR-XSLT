...
DisplayMessage  ShowMessage GetChildNodes GetParentNode GetAllChildWithName GetNewIdGroup GetObjFromIds StringJoin

GetAnewId InsertNodeAtt CopyNodeWithNewId  FilterNodes  GetAllKeyIds

DoUnion  DoIntersection  DoValueIntersection DoMinus DoValueExcept DoDistinct

GetSortedBy  SortedByNumber  MergeDiffOps  GetStringAsList IsStringInList TrimStringAll

GetPosX  GetPosY GetNodeAbsPos GetNewPosXY  
...
<!-- Merge the different part into the orginal model-->
<xsl:function name="my:MergeDiffOps">
	<xsl:param name="oldnode"/> 
    <xsl:param name="diffs"/> 

  <xsl:variable name="sss" as="item()*">
         <xsl:apply-templates mode="DOEleToLinkCopy" select="$diffs"/>
  </xsl:variable>  
  
  <xsl:variable name="vvv" as="item()*">
        <xsl:choose>
         <xsl:when test="count($sss//*[@xmiDiff]) > 0">
			<xsl:apply-templates mode="XMI_Merge" select="$oldnode">
			   <xsl:with-param name="xmiDiffList" select="$sss//*[@xmiDiff]"/>
			</xsl:apply-templates>
         </xsl:when>
         <xsl:otherwise>
            <xsl:sequence select="$oldnode"/>
         </xsl:otherwise>
      </xsl:choose>
  </xsl:variable>
 
  <xsl:sequence select="$vvv"/>

</xsl:function>

<xsl:function name="my:GetLinksToAtts">
  <xsl:param name="arg1"/> 

  <xsl:variable name="sss" as="item()*">
         <xsl:apply-templates mode="DOEleToLinkCopy" select="$arg1"/>
  </xsl:variable>  
 
  <xsl:sequence select="$sss"/>
...
<xsl:template name="copyAttsNoXmiDiff">	
   ...
   <xsl:function name="my:GetAnewId">
      <xsl:param name="in"/>
      <xsl:param name="pos"/>
      <xsl:param name="sufix"/>
      ...
      <xsl:sequence select="concat($p2,$sufix,$p1)"/>
   </xsl:function>
   
<xsl:function name="my:InsertNodeAtt">
	<xsl:param name="node"/>
...
<xsl:function name="my:ResetNodeName">
	<xsl:param name="node"/>
...
<xsl:function name="my:GetAllKeyIds">
	<xsl:param name="node"/>

	<xsl:sequence select="$node//@*[name()=$xmiKeyVar]"/>
   
</xsl:function>
   
<xsl:function name="my:CopyNodeWithNewId">
	<xsl:param name="node"/>
    <xsl:param name="pos"/>
    <xsl:param name="sufix"/>
	<xsl:param name="idlist"/>
    
    <xsl:variable name="idlst" select="if (exists($idlist)) then $idlist else $node//@*[name()=$xmiKeyVar]"/>
 
    <xsl:variable name="vvv" as="item()*">
	<xsl:for-each select="$node">
		 <xsl:if test="exists(name()) and not(name()='')">
		 <xsl:element name="{name()}">
		 <xsl:for-each select="@*">
			 <xsl:choose>
				<xsl:when test="name()=$xmiKeyVar">
					<xsl:attribute name="{$xmiKeyVar}" select="my:GetAnewId(.,$pos,$sufix)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:variable name="attlst" select="my:GetStringAsList(.)"/>
					<xsl:variable name="cpart" select="my:DoValueIntersection2($idlst,$attlst)"/>
					<xsl:choose>
						<xsl:when test="exists($cpart)">
							<xsl:variable name="aaa" as="item()*">
							<xsl:for-each select="$attlst">
								<xsl:choose>
									<xsl:when test="index-of($idlst,.) > 0">
										<xsl:sequence select="my:GetAnewId(.,$pos,$sufix)"/>
									</xsl:when>									
									<xsl:when test="index-of($idlst,substring-after(.,'#')) > 0">
										<xsl:variable name="nida" select="my:GetAnewId(substring-after(.,'#'),$pos,$sufix)"/>
										<xsl:sequence select="concat(substring-before(.,'#'),'#',$nida)"/>
									</xsl:when>									
									<xsl:otherwise>
										<xsl:sequence select="."/>
									</xsl:otherwise>
							...
							<xsl:attribute name="{name()}" select="string-join($aaa,' ')"/>	
						</xsl:when>
						<xsl:otherwise>
							<xsl:sequence select="."/>
						</xsl:otherwise>
		  ...
		  <xsl:copy-of select="text()"/> 
		  <xsl:variable name="newnds" select="child::node()"/>
		  <xsl:if test="exists($newnds)">
			  <xsl:sequence select="my:CopyNodeWithNewId($newnds,$pos,$sufix,$idlist)"/>		  
		  </xsl:if>
	   </xsl:element>
	   </xsl:if>	
    </xsl:for-each>
...
<xsl:function name="my:FilterNodes">
	<xsl:param name="nodes"/>
	<xsl:param name="delidlist"/>

    <xsl:variable name="vvv" as="item()*">
	<xsl:for-each select="$nodes">
		<xsl:if test="not(index-of($delidlist,./@*[name()=$xmiKeyVar]) > 0)">
		 <xsl:if test="exists(name()) and not(name()='')">
		 <xsl:element name="{name()}">
			<xsl:for-each select="@*">
				<xsl:variable name="attlst" select="my:GetStringAsList(.)"/>
				<xsl:choose>
					<xsl:when test="exists(my:DoValueIntersection2($delidlist,$attlst))">
						<xsl:variable name="aaa" as="item()*">
						<xsl:for-each select="$attlst">
							<xsl:if test="not(index-of($delidlist,.) > 0 or index-of($delidlist,substring-after(.,'#')) > 0)">
								<xsl:sequence select="."/>							
							</xsl:if>
						</xsl:for-each>
						</xsl:variable>
		    ...
		    <xsl:variable name="newnds" select="child::node()"/>
		    <xsl:if test="exists($newnds)">
			  <xsl:sequence select="my:FilterNodes($newnds,$delidlist)"/>		  
		    </xsl:if>
	   </xsl:element>	
	   </xsl:if>
	   </xsl:if>
...
<xsl:function name="my:DoValueIntersection2">
	<xsl:param name="os"/>
	<xsl:param name="ts"/>

	<xsl:for-each select="$os">
		<xsl:variable name="oo" select="."/>
		<xsl:if test="exists($oo) and not($oo='')">
		<xsl:for-each select="$ts">
			<xsl:if test=".=$oo or substring-after(.,'#')=$oo">
				<xsl:sequence select="$oo"/>
			</xsl:if>		
		</xsl:for-each>
		</xsl:if>
...
<xsl:function name="my:DoValueIntersection">
	<xsl:param name="os"/>
	<xsl:param name="ts"/>
...
<xsl:function name="my:TrimStringAll" as="xs:string">
  <xsl:param name="arg"/> 
